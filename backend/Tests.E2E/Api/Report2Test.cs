...
using PeruControl.Controllers.Reports; // For Report2DTO, UpdateReport2DTO
...
public class Report2Test
{
    ...
    [TestMethod]
    public async Task GetReport2_ShouldReturnDefaultReport_ForNewAppointmentAsync()
    {
        ...
        if (reportResponse.StatusCode != HttpStatusCode.OK)
        {
            ...
            Assert.Fail(
                $"Failed to get Report2. Status: {reportResponse.StatusCode}. Content: {errorContent}"
            );
        }
        Assert.AreEqual(HttpStatusCode.OK, reportResponse.StatusCode);

        var reportDto = await reportResponse.Content.ReadFromJsonAsync<Report2DTO>();
        Assert.IsNotNull(reportDto, "Report2DTO should not be null.");
        Assert.AreNotEqual(Guid.Empty, reportDto.Id, "Report2 Id should not be empty.");
        Assert.IsNull(reportDto.SigningDate, "SigningDate should be null for a new Report2.");
        Assert.IsNotNull(reportDto.Content, "Content should not be null for a new Report2.");
        Assert.IsFalse(reportDto.Content.Any(), "Content should be empty for a new Report2.");
    }

    [TestMethod]
    public async Task UpdateReport2_ShouldUpdateFieldsAsync()
    {
        var accessToken = await AuthTest.GetAccessTokenAsync();
        using var httpClient = new HttpClient();
        httpClient.DefaultRequestHeaders.Authorization =
        ...
        var testContentString = "This is the updated Report2 text area content.";
        var updateDto = new UpdateReport2DTO
        {
        ...
        if (patchResponse.StatusCode != HttpStatusCode.NoContent)
        {
            ...
            Assert.Fail(
                $"Failed to update Report2. Status: {patchResponse.StatusCode}. Content: {errorContent}"
            );
        }
        ...
        if (getResponse.StatusCode != HttpStatusCode.OK)
        {
            ...
            Assert.Fail(
                $"Failed to get Report2 after update. Status: {getResponse.StatusCode}. Content: {errorContent}"
            );
        }
        Assert.AreEqual(HttpStatusCode.OK, getResponse.StatusCode);

        var updatedReportDto = await getResponse.Content.ReadFromJsonAsync<Report2DTO>();
        Assert.IsNotNull(updatedReportDto, "Updated Report2DTO should not be null.");

        Assert.IsNotNull(
            updatedReportDto.SigningDate,
            "Updated SigningDate should not be null for Report2."
        );
        Assert.AreEqual(
            updateDto.SigningDate.Value.Date,
            updatedReportDto.SigningDate.Value.Date,
            "SigningDate for Report2 was not updated correctly."
        );
        Assert.IsNotNull(
            updatedReportDto.Content,
            "Updated Content list should not be null for Report2."
        );
        Assert.IsTrue(
            updatedReportDto.Content.Any(),
            "Updated Content list should not be empty for Report2."
        );
        ...
        Assert.IsInstanceOfType(
            firstSection,
            typeof(TextArea),
            "First content section for Report2 should be a TextArea."
        );
        var textAreaSection = (TextArea)firstSection;
        Assert.AreEqual(
            testContentString,
            textAreaSection.Content,
            "TextArea content for Report2 was not updated correctly."
        );
    }
}
