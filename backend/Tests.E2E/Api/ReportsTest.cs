...
public class ReportsTest
{
    ...
    [TestMethod]
    public async Task GetCompleteReport_ShouldReturnDefaultReport_ForNewAppointmentAsync()
    {
    ...
    [TestMethod]
    public async Task UpdateCompleteReport_ShouldUpdateFieldsAsync()
    {
        var accessToken = await AuthTest.GetAccessTokenAsync();
        using var httpClient = new HttpClient();
        httpClient.DefaultRequestHeaders.Authorization =
        ...
        var updatedReportDto = await getResponse.Content.ReadFromJsonAsync<CompleteReportDTO>();
        Assert.IsNotNull(updatedReportDto, "Updated CompleteReportDTO should not be null.");
