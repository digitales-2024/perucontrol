			...
			steps {
				...
				dir("backend") {
					catchError(buildResult: "UNSTABLE", stageResult: "FAILURE") {
						sh "docker run --network perucontrol-network-ci-${BUILD_REF} -e BASE_URL=${BASE_URL} -e API_URL=${API_URL} -v \$(pwd):/tests digitalesacide/playwright-dotnet9-noble:latest bash -c 'cd /tests && dotnet test"
					}
				}
			}
			post {
