...
// {
//     const { setValue, watch, register } = useFormContext(); // Usar el contexto del formulario
//     const appointments = watch("appointments") || []; // Leer las fechas programadas desde el formulario
...
//         const dateExists = appointments.some((item: ScheduledDate) => format(new Date(item.date), "yyyy-MM-dd") === format(newDate, "yyyy-MM-dd"));
...
//         const updatedDates = [...appointments, { id: crypto.randomUUID(), date: newDate }];
//         setValue("appointments", updatedDates); // Actualizar el estado compartido
//         setNewDate(undefined); // Limpiar el campo de nueva fecha
//         toast.success("Fecha agregada correctamente");
//     };

//     const handleDeleteDate = (id: string) =>
//     {
//         const updatedDates = appointments.filter((date: ScheduledDate) => date.id !== id);
//         setValue("appointments", updatedDates); // Actualizar el estado compartido
...
//         const dateExists = appointments.some((item: ScheduledDate) => item.id !== editingDate.id && format(new Date(item.date), "yyyy-MM-dd") === format(newDate, "yyyy-MM-dd"));
...
//         const updatedDates = appointments.map((date: ScheduledDate) => (date.id === editingDate.id ? { ...date, date: newDate } : date));

//         setValue("appointments", updatedDates); // Actualizar el estado compartido
//         setIsEditDialogOpen(false); // Cerrar el diálogo
//         setEditingDate(null); // Limpiar el estado de edición
...
//         setValue("appointments", newScheduledDates); // Actualizar el estado compartido
...
//                                             {appointments
//                                                 .sort((a: ScheduledDate, b: ScheduledDate) =>
//                                                 {
...
//                     {/* Inputs ocultos para registrar las fechas programadas */}
//                     {appointments &&
//             appointments.map((scheduledDate: ScheduledDate, index: number) => (
//                 <div key={scheduledDate.id} className="hidden">
//                     <input type="hidden" {...register(`appointments.${index}.id` as const)} value={scheduledDate.id} />
//                     <input
//                         type="hidden"
//                         {...register(`appointments.${index}.date` as const)}
//                         value={
//                             scheduledDate.date instanceof Date
//                                 ? scheduledDate.date.toISOString()
//                                 : new Date(scheduledDate.date).toISOString()
//                         }
//                     />
//                 </div>
//             ))}
