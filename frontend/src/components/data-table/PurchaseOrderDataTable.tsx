...
// Utilidad para obtener valores de campos anidados
...
interface PurchaseOrderTableProps<T> {
...
export function PurchaseOrderTable<T extends object>({
    columns,
    data,
    statusOptions,
    statusField,
...
}: PurchaseOrderTableProps<T>)
{
    ...
    useEffect(() =>
    {
        ...
        const timeout = setTimeout(() =>
        {
            ...
            // --- Buscador mejorado: busca todas las palabras en cualquier orden y soporta campos anidados ---
            if (searchTerm && searchFields && searchFields.length > 0)
            {
                ...
                {
                    const fieldValue = getNestedValue(item, field as string).toString()
                        .toLowerCase();
    ...
    // Solo dispara el loader cuando cambian estos filtros principales:
                    ...
                    <TableBody>
                        {isLoading ? (
                            <TableRow>
                                <TableCell
                                    colSpan={
                                        columns.length + ((actionButtons?.length || dropdownActions?.length) ? 1 : 0)
                                    }
